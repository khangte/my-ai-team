...
- README 갱신·오탈자 수정·특정 커밋 반영처럼 해석 여지 없는 단순 문서 작업은 lead가 직접 처리한다 — 배분 왕복과 Opus 턴을 아끼기 위함이다
...
- `superpowers:brainstorming` — 요구가 불명확하거나 설계 대안이 여러 갈래일 때만 쓴다
  ...
  - 이 스킬은 "유저 승인 전 구현 금지" 게이트를 걸고 승인을 요구하는데, **여기서 승인 주체는 lead(:0.0)다** — 사용자에게 직접 묻지 않는다
  - 설계안이 서면 `say :0.0`으로 승인을 요청하고, 회신을 받은 뒤 다음 단계로 넘어간다
...
say :0.0 "[architect] {기능명} 리뷰 판정: {승인/수정필요}. {항목 수}건. docs/architect-review/{파일명}"
```
