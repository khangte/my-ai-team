...
- **같은 세션에서 이미 읽은 파일을 다시 Read하지 않는다**
  - 컨텍스트에 한 번 들어간 내용은 그 세션의 남은 모든 턴에서 다시 청구된다
  - 내용이 기억나지 않으면 다시 읽지 말고 앞선 대화에서 찾는다
- **300줄이 넘는 파일은 `offset`/`limit`으로 필요한 구간만 읽는다**
  - 어디를 봐야 할지 모르면 Grep으로 줄 번호를 먼저 찾고 그 주변만 읽는다

### 보고 규칙 (모든 역할 공통)

- **`say`를 실제로 실행해야 보고다**
  - 응답 텍스트는 파인 밖으로 나가지 않으므로 "완료했습니다"라고 쓰는 것은 보고가 아니다
