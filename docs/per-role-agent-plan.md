...
다른 에이전트를 띄우는** 구조를 설계한다.

핵심 동기는 **코드를 쓰는 모델과 판단하는 모델을 분리**하는 것이다. 같은 모델끼리는
맹점도 공유하기 때문이다. 어느 역할을 Codex로 돌릴지는 프로젝트마다 다르고, 실제로
저장소 기본값도 여러 번 바뀌었다 — 이 설계가 지탱해야 하는 것은 특정 배정이 아니라
**배정을 바꿔도 `say` 큐·유휴 판정·보고 체계가 동일하게 동작한다**는 성질이다.

현재 저장소 기본값은 `team/config.sh`의 `MEMBER_AGENTS`에 있다(집필 시점: developer만
Codex, `gpt-5.6-sol`). 문서가 아니라 그 배열이 사실이다.
...
- 어느 역할을 Codex로 띄워도 `say` 큐·유휴 판정이 지금과 동일하게 동작한다.
...
- 역할별 기본 공급자 배정 자체를 이 문서가 확정하지 않는다 — `team/config.sh`의
  `MEMBER_AGENTS`가 소유한다.
...
| 시점                       | 훅                 | 동작            |
| -------------------------- | ------------------ | --------------- |
| 턴 시작                    | `UserPromptSubmit` | busy 마커 생성, 보고 마커 초기화 |
| 턴 종료                    | `Stop`             | busy 마커 삭제, 보고 마커 소비 |
| `/clear`·`/compact`·재시작 | `SessionStart`     | stale busy 마커 삭제 |

보고 마커(`/tmp/team-say`) 초기화가 `UserPromptSubmit`에 얹혀 있다는 점이 뒤의
§"보고 마커의 공급자 비대칭"으로 이어진다 — Codex에는 이 훅이 없다.
...
- 혼합 배정(당시 기본값: architect·reviewer가 Codex)에서 `MEMBER_AGENTS`·역할별 모델·
  추론 수준이 의도대로 해석됨을 회귀 테스트로 확인했다. 이후 기본 배정이 바뀌었지만
  해석 경로는 배정과 무관하므로 결과는 유효하다.
...
보고 마커(`/tmp/team-say/{state_key}`)는 수신자를 구분하지 않는 **"이번 턴에
보고했나" 1비트**다. Stop 훅이 이 마커를 보고 종료 신호를 보낼지 정하므로,
턴마다 초기화되어야 한다. 그러지 않으면 한 턴에 두 번 보고했을 때(`say architect`
+ `say lead`) Stop 훅이 한 번만 소비하고 나머지 상태가 다음 턴으로 넘어가,
보고를 하고도 "미보고" 신호가 나간다. 실측으로 확인한 오탐이다
([pane-messaging.md](pane-messaging.md) 10단계).

초기화 지점이 공급자마다 다르다.

| 공급자 | 초기화 지점 | 비고 |
| --- | --- | --- |
| Claude | `UserPromptSubmit`(턴 시작) + `Stop`(소비) | 이중 안전장치 |
| Codex | `Stop`(소비)만 | `UserPromptSubmit` 훅 자체가 없다 |

Codex 파인은 Stop 훅의 삭제가 유일한 초기화 지점이다. 한 턴 안에서 마커가
여러 번 찍혀도 파일은 하나이므로 Stop에서 무조건 지우면 1비트 의미가 유지된다 —
현재 구현이 그렇다. 다만 Claude 쪽의 턴 시작 초기화가 주는 여유가 없으므로,
Codex 파인의 Stop 훅을 건드릴 때 이 삭제를 조건부로 바꾸지 않아야 한다.

`CLAUDE.md`·`AGENTS.md`의 "한 턴에 lead로 두 번 이상 `say` 하지 않는다" 규칙이
지침 쪽에서도 같은 상황을 막는다.

### 역할 산출물의 품질 변동

모델을 바꾸면 그 역할의 문체와 밀도가 달라진다. 리뷰라면 지적 밀도가, 설계라면 대안
제시 폭이 바뀐다. 해당 `team/{역할}.md`가 요구하는 출력 형식이 바뀐 공급자에서도
지켜지는지 실제 산출물 몇 건으로 확인하고, 필요하면 지침을 공급자 중립적으로 다듬는다.
지침을 특정 공급자에 맞춰 특화하지는 않는다 — 배정은 바뀌지만 지침은 남는다.

보고 프로토콜도 같은 기준으로 본다. `CLAUDE.md`/`AGENTS.md`의 보고 규칙(사본 금지,
사안 단위 보고)은 양쪽에 동일하게 들어가지만, Claude는 프로젝트 루트 `CLAUDE.md`를,
Codex는 `AGENTS.md`를 읽는다. **두 파일을 함께 갱신하지 않으면 혼합 팀에서 한쪽
공급자만 옛 규칙으로 동작한다.**

## 최종 완료 기준

- [x] `MEMBER_AGENTS`로 파인별 에이전트를 지정할 수 있다.
- [x] 혼합 팀에서 `say` 큐와 유휴 판정이 단일 공급자 팀과 동일하게 동작한다
      (실 tmux 파인으로 busy 마커 생성→Stop 훅→마커 삭제 전 과정 확인).
...
- Codex 파인의 프롬프트·툴 JSONL 로깅. `log_cmd`는 `start_claude_in_pane`에만 있어
  `.claude-logs/`에 Codex 파인의 기록이 남지 않는다. 혼합 팀에서 보고 경로를 실측할 때
  Codex 파인 구간만 공백으로 비는 원인이다(실제로 겪음 — developer 파인의 발신 시점을
  lead 수신 로그로 역추정해야 했다).
- 보고 프로토콜 변경([pane-messaging.md](pane-messaging.md) 10단계)이 Codex 파인에서도
  동일하게 지켜지는지 확인. Claude 파인에서는 실측했지만 Codex 쪽은 위 로깅 부재로
  같은 방식의 검증이 안 된다.
